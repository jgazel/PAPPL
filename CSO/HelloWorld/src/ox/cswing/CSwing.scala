...
import  java.awt.{Dimension, Component, Container, BorderLayout}
...
package ox.cswing
{
  ...
  case  object Horizontal extends Orientation(SwingConstants.HORIZONTAL)      
    ...
    /**
          The classes of this package provide a small selection of
          widgets derived from Swing and set up for easy declarative
          specification of simple Interfaces. There are two main
          differences:
          ...
          1. The widgets have a number of convenience methods
             ...
             attributes.
          <p>
          2. Listeners are not used. Instead ''action'' widget
          ...
          <tt>action:Unit=>Unit</tt> method at the point of
  ...
  */
  ...
  { var North, South, East, West, Center: JComponent = _
    ...
    {  if (North!=null)  add(North, "North")
       if (South!=null)  add(South, "North")
       if (East!=null)   add(East, "North")
       if (West!=null)   add(West, "North")
       if (Center!=null) add(Center, "North")
       super.doLayout
    }
  }
  ...
  */
  abstract class  JSlider(or: Orientation, min: Int, max:Int, value: Int)
  extends javax.swing.JSlider(or.rep, min, max, value)   
  with    Widget[JSlider]
  {  private var instant = false
     private var _onChange: Unit => Unit = _
     def change(_value: Int) : Unit
     ...
     (   addChangeListener
         ( new ChangeListener()
           { def stateChanged(ev: ChangeEvent)  
             { if (instant || !getModel.getValueIsAdjusting) change(getValue) } 
           }
         )
     )
   ...
   */
   ...
   /**
      This trait is used to construct MouseInputListeners that can be
      added to (C)Swing components to capture mouse events.
   */
                
   trait MouseInputProxy 
   extends javax.swing.event.MouseInputListener
   { import ox.CSO._
     import ox.cswing.event._
     val name:  String
     val out:   ![Event]
   
     def mouseDragged(ev: MouseEvent)
     { out!MouseDragged(name, (ev.getX, ev.getY), 
                              ev.getButton, ev.getModifiersEx) }
     
     def mouseMoved(ev: MouseEvent)
     { out!MouseMoved(name, (ev.getX, ev.getY), 
                            ev.getButton, ev.getModifiersEx) }
     
     def mousePressed(ev: MouseEvent)
     { out!MousePressed(name, (ev.getX, ev.getY), 
                              ev.getButton, ev.getModifiersEx) }
     
     def mouseReleased(ev: MouseEvent)
     { out!MouseReleased(name, (ev.getX, ev.getY), 
                               ev.getButton, ev.getModifiersEx) }
     
     def mouseClicked(ev: MouseEvent) { }
     
     def mouseEntered(ev: MouseEvent) 
     { out!MouseEntered(name, (ev.getX, ev.getY), 
                              ev.getButton, ev.getModifiersEx) }
    
     def mouseExited(ev: MouseEvent)  
     { out!MouseExited(name, (ev.getX, ev.getY), 
                             ev.getButton, ev.getModifiersEx) }
   }
   
   /** Constructor methods for common widgets */
   object Widget
   {  
      ...
      def  menuItem(name: String)(act: => Unit) : JMenuItem = 
       ...
       def checkBox(name: String)(act: Boolean => Unit) = 
           new JCheckBox(name, act)
       
       def checkBox(name: String, state: Boolean)(act: Boolean => Unit) = 
           new JCheckBox(name, act) withSelected state
       
       def button(name: String)(act: => Unit) = 
           new JButton(name, { case ()=>act })
       ...
       def label(text: String, icon: Icon) = 
           new JLabel(text, icon)
       
       def label(text: String) = 
           new JLabel(text)
       
       def buttonGroup[W <: ButtonWidget[Any]](buttons: Seq[W]) = 
           new ButtonGroup(buttons)

       def slider(or: Orientation, min: Int, max:Int, value: Int)
                 (act: Int=>Unit) =
           new JSlider(or, min, max, value) 
           { def change(_value: Int) = act(_value)
           }   
